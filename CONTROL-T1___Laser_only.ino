...
inline void Super_Sonic_to_Midi(
                                byte &MIDI_Channel,
                                uint8_t &LATENCY,
                                uint16_t duration,
                                uint16_t &DUR_MIN,
                                ...
                                uint8_t &SS_Ctrl,
                                uint8_t *Value) {
  bVlx = vlx.get(DUR_MIN, DUR_MAX);
  //Serial.println(bVlx);
  if ((bVlx >= 0 && bVlx <= 127) && (bVlx != lbVlx)) {
    controlChange(MIDI_Channel, SS_Ctrl, bVlx, Value);
    lbVlx = bVlx;
  }
}

inline void PotentiomertoMidi(uint16_t *anVal, uint16_t *lastAnVal, uint8_t ctrlNum, char anPIN, uint8_t POT_CTRL, byte MIDI_Channel, uint8_t *Value) {
